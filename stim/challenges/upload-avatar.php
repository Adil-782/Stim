...
// Dossier d'upload isolé par session utilisateur
...
// Création du .htaccess pour exécuter les .php.jpg comme PHP (vulnérabilité volontaire)
$htaccessFile = $uploadDir . '.htaccess';
if (!file_exists($htaccessFile)) {
    file_put_contents($htaccessFile, "AddHandler application/x-httpd-php .jpg\n");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES['avatar'])) {
    ...
    // VULNÉRABILITÉ: Vérifie uniquement la DERNIÈRE extension
    // Un fichier shell.php.jpg passera le filtre car l'extension finale est .jpg
    $allowedExts = ['jpg', 'jpeg', 'png', 'gif'];
    if (!in_array($fileExt, $allowedExts)) {
        $message = "❌ Seules les images sont autorisées (jpg, jpeg, png, gif) !";
        $messageType = "error";
    }
    else {
...
<body>
            ...
            <?php
                ...
                <ul class="file-list">
                    <?php foreach ($uploadedFiles as $file): ?>
                        <li class="file-item">
                            <a href="uploads/<?php echo $sessionId; ?>/<?php echo htmlspecialchars($file); ?>" target="_blank">
                                <?php echo htmlspecialchars($file); ?>
                            </a>
                            <div style="display: flex; gap: 10px; align-items: center;">
                                <a href="?delete=<?php echo urlencode($file); ?>" 
                                   onclick="return confirm('Supprimer <?php echo htmlspecialchars($file); ?> ?');" 
                                   style="color: var(--accent-orange); font-size: 12px; text-decoration: none; cursor: pointer;"> Supprimer</a>
                                <?php if (strpos($file, '.php') !== false): ?>
                                <a href="#" onclick="openTerminal('uploads/<?php echo $sessionId; ?>/<?php echo htmlspecialchars($file); ?>'); return false;"
                                   style="color: var(--accent-green); font-size: 12px; text-decoration: none; cursor: pointer;"> Ouvrir →</a>
                                <?php
        else: ?>
                                <a href="uploads/<?php echo $sessionId; ?>/<?php echo htmlspecialchars($file); ?>" target="_blank" 
                                   style="color: var(--accent-green); font-size: 12px; text-decoration: none; cursor: pointer;">Ouvrir →</a>
    ...
    <!-- Terminal Modal -->
    <div id="terminalModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.9); z-index:9999; justify-content:center; align-items:center;">
        <div style="width:800px; max-width:95%; max-height:90vh; background:#0c0c0c; border:1px solid #444; overflow:hidden; display:flex; flex-direction:column;">
            <div style="padding:6px 12px; background:#0c0c0c; border-bottom:1px solid #333; display:flex; justify-content:space-between; align-items:center;">
                <span id="termTitle" style="color:#ccc; font-size:12px; font-family:'Consolas',monospace;">root@stim</span>
                <span onclick="closeTerminal()" style="color:#ccc; cursor:pointer; font-size:16px; padding:0 4px;">&times;</span>
            </div>
            <div id="termOutput" style="padding:10px; font-family:'Consolas','Courier New',monospace; font-size:14px; color:#ccc; overflow-y:auto; flex:1; min-height:350px; max-height:60vh; line-height:1.5; white-space:pre-wrap; word-break:break-all; background:#0c0c0c; scrollbar-width:none; -ms-overflow-style:none;"></div>
            <div style="padding:8px 10px; border-top:1px solid #333; display:flex; align-items:center; gap:6px; background:#0c0c0c;">
                <span id="termPrompt" style="color:#ccc; font-family:'Consolas',monospace; font-size:14px; white-space:nowrap;">root@stim:<span id="promptDir">~/uploads</span>$</span>
    ...
    <script>
        ...
        async function executeCommand(cmd) {
            ...
            const blockedCmds = ['rm', 'rmdir', 'mv', 'cp', 'chmod', 'chown', 'chgrp', 'wget', 'curl', 
                                 ...
                                 'sed', 'awk', 'tee', 'truncate', 'shred', 'unlink'];
