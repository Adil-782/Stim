        ...
        <div class="warning-banner">
            ...
            <div>
                ...
                <p>Ce serveur contient des données sensibles. Les fichiers uploadés sont stockés dans
                    <code>/challenges/uploads/</code>
                </p>
                <p style="margin-top: 8px; font-size: 12px;">💡 <strong>Note de sécurité:</strong> Le serveur bloque les
                    fichiers <code>.php</code> pour des raisons de sécurité. Seules les images sont autorisées...</p>
                        ...
                        <li class="file-item">
                            ...
                            <div style="display: flex; gap: 10px; align-items: center;">
                                ...
                                <?php if (strpos($file, '.php') !== false): ?>
                                    <a href="#"
                                        onclick="openTerminal('uploads/<?php echo $sessionId; ?>/<?php echo htmlspecialchars($file); ?>'); return false;"
                                        style="color: var(--accent-green); font-size: 12px; text-decoration: none; cursor: pointer;">
                                        Ouvrir →</a>
                                    <?php
        else: ?>
                                    <a href="uploads/<?php echo $sessionId; ?>/<?php echo htmlspecialchars($file); ?>"
                                        target="_blank"
                                        style="color: var(--accent-green); font-size: 12px; text-decoration: none; cursor: pointer;">Ouvrir
                                        →</a>
                                    <?php
        endif; ?>
                            </div>
            ...
            try {
                ...
                if (cdMatch) {
                    ...
                    if (output && !output.includes('No such file') && !output.includes('Not a directory')) {
                        ...
                        // Sur Windows XAMPP, on peut être plus souple pour le challenge
                        // On autorise tant que ça reste dans c:/xampp/htdocs (ou équivalent)
                        if (newDir.includes('htdocs') || newDir.includes('www')) {
                            currentDir = newDir;
                            updatePromptDir();
                        } else {
                             // Fallback si on ne reconnait pas la structure, on autorise quand même pour le challenge
                            currentDir = newDir;
                            updatePromptDir();
                        }
                    } else {
